...
boolean isMaster = false; // correspond à l'arduino connecté à l'USB Camera, 
...
int stepperGoal = -stepperRevolution;//-stepResolution;
int stepperPos = 0;
...
void loop()
{
  ...
  else if ( (isMaster && camConnected == 1) || (isMaster == false))
  {
    ...
    if (runStepper == 0){
      ...
      //strip.setPixelColor(0, 0,255,0); //// pause == vert au centre
      strip.show();
    } 
...
void checkEthernet() {
  ...
  if(packetSize)
  {
    ...
    // édition de la vitesse en live
    else if(strcmp(pch, "setSpeed") == 0){
      while(pch != NULL){
        // convertit la vitesse (chaine notation hex) en int
        int newSpeed = strtol(pch,NULL,16);
